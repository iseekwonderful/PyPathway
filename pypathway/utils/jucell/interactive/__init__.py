...
class Interactivable:
    '''
    A class accept the interactive option and serialize the option, also means the corresponding js can parse these
    elements
...
class IntegrationOptions:
    ...
    def set(self, id_list, visualize_option_list):
        ...
        if len(id_list) != len(visualize_option_list):
            raise Exception("Length of id_list != length of visualize_option_list")
        for x in range(len(id_list)):
            if not isinstance(visualize_option_list[x], VisualizationOption):
                raise Exception("This member of list visualize_option_list should be the instance of VisualizeOption!")
...
class VisualizationOption:
    def __init__(self, default=None, over=None, click=None, right=None):
        self.default = {}
        if default:
            for x in default:
...
class Delete(InteractiveAction):
    def __init__(self, delete_edges=True):
        InteractiveAction.__init__(self)
        self.delete_edges = delete_edges

    @property
    def json(self):
        return {'delete': {'delete_edge': self.delete_edges}}


class Expend(InteractiveAction):
    ...
    @property
    def json(self):
        return {'expend': {'level': self.level}}



class CXTMenu(InteractiveAction):
    def __init__(self, setting: dict):
        '''
        Init the cxtmenu, a circle menu recommend to be used while right click
        RECOMMEND in right click!
    ...
    @property
    def json(self):
        return {'cxtmenu': {k: v.json for k, v in self.setting.items()}}
